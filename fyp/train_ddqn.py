...
class DeepQLearningAgent:
    ...
    def __init__(self, env):
        ...
        # self.dqn_model.add(Dense(512, input_dim=self.NUM_STATE_ACTION, activation='relu'))  # ROW_SIZE*COL_SIZE*2+1 (first 2 layers and action)
        # self.dqn_model.add(Dropout(0.2))
        # self.dqn_model.add(Dense(1024, activation='relu'))
        # self.dqn_model.add(Dropout(0.2))
        # self.dqn_model.add(Dense(512, activation='relu'))
        # self.dqn_model.add(Dropout(0.2))
        # self.dqn_model.add(Dense(1, activation='linear'))
        # self.dqn_model.compile(loss='mse', optimizer=Adam())
        # self.dqn_model.save('trained_models/8x8_ddqn.h5')
        self.dqn_model = load_model('trained_models/8x8_ddqn.h5')
        self.target_model = load_model('trained_models/8x8_ddqn.h5')
    ...
    def get_state_action(self, state, action):
        a = np.zeros(128)
        a[action] = 1
        return np.append(state.flatten(), a)
    ...
    def best_action(self, state, actions):
        ...
        value = self.dqn_model.predict(np.reshape(state_action, [1, self.NUM_STATE_ACTION]))
        max_value = value
        max_action = action
    ...
    def learn(self, i_step):
        ...
        for i in indexes:
            ...
            state_action = self.get_state_action(state, action)
            predicted_value = self.dqn_model.predict(np.reshape(state_action, [1, self.NUM_STATE_ACTION]))
            if done:
                value = reward
            else:
                next_actions = self.action_filter(next_state)
                best_next_action, _ = self.best_action(next_state, next_actions)
                next_best_state_action = self.get_state_action(next_state, best_next_action)
                max_value = self.target_model.predict(np.reshape(next_best_state_action, [1, self.NUM_STATE_ACTION]))
        ...
        self.dqn_model.fit(np.array(x_batch), np.array(y_batch), batch_size=len(x_batch), verbose=0)
...
if __name__ == '__main__':
    ...
    for i_episode in range(NUM_EPISODE):
        ...
        if i_episode % 10 == 0:
            ...
            f.write('%d, %d, %f, %d, %f, %f \n' % (i_episode, sum_reward, running_reward, agent.epsilon, mean_square_loss, running_loss))
            agent.save_model()
            agent.target_model.set_weights(agent.dqn_model.get_weights())
    f.close()
