...
class DeepQLearningAgent:
    ...
    def __init__(self, env):
        ...
        self.model.add(Dense(500, input_dim=290, activation='relu'))  # ROW_SIZE*COL_SIZE*2+2 (first 2 layers and 2 remaining number)
        self.model.add(Dense(800, activation='relu'))
        self.model.add(Dense(319, activation='linear'))
    ...
    def preprocess_state(self, state):
        return np.reshape(state, [1, 290])

    def act(self, state):
        if np.random.random() <= self.epsilon:
            # Explore.
            # action = self.env.action_space.sample()
            action = random.randint(0, 318)
        else:
            # Exploit.
    ...
    def save_model(self):
        self.model.save('trained_models/12x12.h5')
