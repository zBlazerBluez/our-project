...
class DeepQLearningAgent:
    ...
    def __init__(self, env):
        ...
        self.model.add(Dense(6100, input_dim=3202, activation='relu')) #ROW_SIZE*COL_SIZE*2+2 (first 2 layers and 2 remaining number)
        self.model.add(Dense(6100, activation='relu'))
        self.model.add(Dense(4407, activation='linear'))
    ...
    def preprocess_state(self, state):
        return np.reshape(state, [1, 3202])

    def act(self, state):
        if np.random.random() <= self.epsilon:
            # Explore.
            # action = self.env.action_space.sample()
            action = random.randint(0,4406)
        else:
            # Exploit.
    ...
    def save_model(self):
        self.model.save('trained_models/big_3202_6100_6100_4407.h5')

if __name__ == '__main__':
    NUM_EPISODE = 500001
    MAX_FRAME = 50

    f = open('logs/big_3202_6100_6100_4407.txt','w')
